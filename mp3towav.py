...
directory = 'D:\\UrduDataset\\NEWSPROGRAM\PTV45Minutes'

i = 0
for filename in os.listdir(directory):
    print(filename)
    if filename.endswith(".wav") or filename.endswith(".mp4") or filename.endswith(".MP4"):
        song = AudioSegment.from_file(directory+'\\'+filename)
        print("exporting "+filename.format(i))
        song.export(
                'F:\\Current Semester\\FYP\\OASRU\\UrduStudioGrade'+'\\'+filename[:len(filename)-4]+'.flac'.format(i),
                format="flac")
        i = i+  1     